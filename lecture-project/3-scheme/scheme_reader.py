...
nil = nil() # Assignment hides the nil class; there is only one instance    这个还挺巧妙的
...
def scheme_read(src):
    ...
    if val == 'nil':
        ...
        # BEGIN PROBLEM 6
        "*** YOUR CODE HERE ***"
        # quotes 包含 3 种情况 核心是构造正确的 Pair 结构
        return Pair(quotes[val], Pair(scheme_read(src), nil))
        # END PROBLEM 6
    elif val not in DELIMITERS:
        # 这里判断 val 不是特殊符号     1. number   2. symbol
        return val
    else:
...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # BEGIN PROBLEM 1
            "*** YOUR CODE HERE ***"
            first = scheme_read(src)    # 读取一个完整的有效表达式  1. number   2. (full expr)    3. symbol
            rest = read_tail(src)
