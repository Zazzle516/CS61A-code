...
class VendingMachine:
    ...
    def __init__(self, good_name, good_price):
        ...
        self.store = 0          # 商品数量初始化为 0
        self.user_money = 0     # 用户金额初始化为 0
    
    def vend(self):
        # 写完反应过来可以把前两个判断合并到 restock() 方法中   不对 restock() 没办法返回 只能老老实实写分支
        if self.store == 0 and self.user_money == 0:
            return 'Inventory empty. Restocking required.'
        elif self.store == 0 and self.user_money < self.good_price:
            return 'Inventory empty. Restocking required. Here is your ${0}.'.format(self.user_money)
        elif self.store > 0 and self.user_money < self.good_price:
            rest = self.good_price - self.user_money
            return 'You must add ${0} more funds.'.format(rest)
        elif self.store > 0 and self.user_money == self.good_price:
            self.user_money -= self.good_price
            self.store -= 1
            return 'Here is your {0}.'.format(self.good_name)
        else:
            self.store -= 1
            change = self.user_money - self.good_price
            self.user_money = 0                 # 找完钱就全还回去了
            return 'Here is your {0} and ${1} change.'.format(self.good_name, change)
    ...
    def add_funds(self, money):
        ...
        if self.store == 0:
            ...
            return 'Current balance: ${0}'.format(self.user_money)
    
    def restock(self, num):
        self.store += num
        return 'Current {0} stock: {1}'.format(self.good_name, self.store)
    ...
    if link == Link.empty:
        ...
        if link.rest is not Link.empty and link.rest.first == value:
            ...
            while link.rest is not Link.empty and link.rest.first == value:
                link = link.rest
            start.rest = link.rest
            remove_all(start.rest, value)
        else:
