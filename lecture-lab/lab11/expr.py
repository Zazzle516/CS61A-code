...
class LambdaExpr(Expr):
    ...
    def eval(self, env):
        print("DEBUG:", "current lambda eval")
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        operator = self.operator.eval(env)  # 我怎么想不到根据 operator 自己的类型去调用 eval()
        if operator is None:
...
class LambdaFunction(Value):
    ...
    def __init__(self, parameters, body, parent):
        ...
        self.parameters = parameters
        self.body = body
        self.parent = parent    # the environment the lambda function was defined in.
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"
        lamdaba_env = self.parent.copy()
        print("DEBUG:", "arguments", arguments)
        for i in zip(self.parameters, arguments):
            print("DEBUG:", "current operand type", type(i[1]))
            lamdaba_env.update({i[0]: i[1]})
        return self.body.eval(lamdaba_env)
...
class PrimitiveFunction(Value):
    ...
    def __init__(self, operator):
        Value.__init__(self, operator)      # 注意这里的 operator 必须是 python function 函数本身
        self.operator = operator
