...
def read_expr(src):
    ...
    if token is None:
        ...
        src.expect(':')
        body = read_expr(src)   # 递归解析 lambda body
        # print("DEBUG:", "current lambda token", LambdaExpr(params, body))
...
def read_call_expr(src, operator):
    while src.current() == '(':
        ...
        src.expect(')')
        print("DEBUG:", "CallExpr Start")
        print("DEBUG:", "operator type", type(operator))
        operator = CallExpr(operator, operands)
        print("DEBUG:", "CallExpr End", operator)
    return operator
