...
def reverse_recursive(lst):
    ...
    if(len(lst) == 0):
        ...
        rest_list = lst[:-1]
        # return [lst[0]] + reverse_recursive(rest_list)  # [1,2,3,4]
        return [lst[-1]] + reverse_recursive(rest_list)
    
    # if not len(lst):
    #     return []
    # # 取出最后一个元素并转变为 list 形式 
...
def distance(city_a, city_b):
    ...
    # pow_dis = pow(abs(a_x - b_x),2) + pow(abs(a_y - b_y),2)
    pow_dis = abs(a_x - b_x)**2 + abs(a_y - b_y)**2
    dis = sqrt(pow_dis)
    return dis
...
def closer_city(lat, lon, city_a, city_b):
    ...
    dis_a = distance(Compare,city_a)
    dis_b = distance(Compare,city_b)
    if dis_a > dis_b:
...
def add_chars(w1, w2):
    ...
    """
    "*** YOUR CODE HERE ***"
    # recursively 前提：w1 一定是 w2 的一部分 也就是 w1 一定能到达终点 
    # 以 w1 的长度为主长度进行递归 
    if(len(w1) == 0):
        if(len(w2) == 0):
            return ''
